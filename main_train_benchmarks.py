...
if __name__ == '__main__': 
    ...
    for s in range(10): 
        ...
        if model_name == 'GT': 
            model = GraphTransformer(in_dim=in_dim, pos_dim=pos_dim, h_dim=h_dim, out_dim=out_dim, num_layers=num_layers, dropout=dropout, pos_method=pos_method, layer_norm_eps=ln).to(device)
        elif model_name == 'GT_LLPE': 
            ...
            model = GraphTransformerX(in_dim=in_dim, pos_dim=pos_dim, h_dim=h_dim, out_dim=out_dim, num_layers=num_layers, pos_method=pos_method).to(device)
        elif model_name == 'HGT': 
            model = HGraphTransformer(in_dim=in_dim, pos_dim=pos_dim, h_dim=h_dim, out_dim=out_dim, num_layers=num_layers, pos_method=pos_method, dropout=dropout).to(device)
        elif model_name == 'HGTX': 
            model = HGraphTransformerX(in_dim=in_dim, pos_dim=pos_dim, h_dim=h_dim, out_dim=out_dim, num_layers=num_layers, pos_method=pos_method).to(device)
        elif model_name == 'H2GT_v1':
            model = H2GraphTransformer_v1(in_dim=in_dim, pos_dim=data.x.shape[0], h_dim=h_dim, out_dim=out_dim, K=K, num_layers=num_layers, dropout=dropout, layer_norm_eps=ln).to(device)
        elif model_name == 'GPS': 
            model = GPS(in_dim=in_dim, pos_dim=pos_dim, h_dim=h_dim, out_dim=out_dim, num_layers=num_layers, pos_method=pos_method, dropout=dropout).to(device)
        elif model_name == 'Graphormer': 
            model = Graphormer(in_dim=in_dim, h_dim=h_dim, out_dim=out_dim, num_layers=num_layers).to(device)
        elif model_name == 'GCN': 
            model = GCN(in_dim=in_dim, h_dim=h_dim, out_dim=out_dim, num_layers=num_layers, pos_dim=pos_dim, dropout=dropout, pos_method=pos_method).to(device)
        elif model_name == 'GCN_LLPE': 
            model = GCN_LLPE(in_dim=in_dim, pos_dim=pos_dim, h_dim=h_dim, out_dim=out_dim, K=K, num_layers=num_layers, dropout=dropout).to(device)
        elif model_name == 'SAGE': 
            model = SAGE(in_dim=in_dim, h_dim=h_dim, out_dim=out_dim, num_layers=num_layers, pos_dim=pos_dim, dropout=dropout, pos_method=pos_method).to(device)
            ...
            model = FAGCN(in_dim=in_dim, h_dim=h_dim, out_dim=out_dim, num_layers=num_layers, pos_dim=pos_dim, dropout=dropout, pos_method=pos_method).to(device)
        elif model_name == 'FAGCN_LLPE': 
            model = FAGCN_LLPE(in_dim=in_dim, pos_dim=pos_dim, h_dim=h_dim, out_dim=out_dim, K=K, num_layers=num_layers, dropout=dropout).to(device)
        elif model_name == 'GCN2': 
            model = GCN2(in_dim=in_dim, h_dim=h_dim, out_dim=out_dim, num_layers=num_layers, pos_dim=pos_dim, dropout=dropout, pos_method=pos_method).to(device)
        elif model_name == 'GCN2_LLPE': 
            model = GCN2_LLPE(in_dim=in_dim, pos_dim=pos_dim, h_dim=h_dim, out_dim=out_dim, K=K, num_layers=num_layers, dropout=dropout).to(device)
        elif model_name == 'MLP': 
            model = MLP(in_dim=in_dim, h_dim=h_dim, out_dim=out_dim, num_layers=num_layers, pos_dim=pos_dim, dropout=dropout, pos_method=pos_method).to(device)
